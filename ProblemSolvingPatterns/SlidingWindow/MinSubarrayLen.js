...
function minSubArrayLen(arr, n) {
    ...
    // declare pointer varliables
    let follower = 0
    let leader = 0
    let total = 0
    let min = Infinity
    // calculate current sum
    while (follower < arr.length) {
        // if sum is less than total
        // move leader pointer over
        if (total < n && leader < arr.length) {
            total += arr[leader]
            leader++
        }
        // if sum is greater than total
        // calculate size
        // move follower point over
        else if (total >= n) {
            min = Math.min(min, leader - follower)
            total -= arr[follower]
            follower++
        }  
        else {
